        ...
        foreach (config('message') as $conf) {
            if ($conf['enable'] !== 1) {
                if ($conf['not_enabled_tips'] && self::$notEnabledTips) { // 仅在存在配置的送信项未启用的情况下提醒
                    system_log(sprintf('Since the "%s" function is not enabled, the mail will not be sent through "%s" this time, although the relevant configuration is detected.', $conf['name'], $conf['name']));
                }
            ...
            if (!$serviceInstance instanceof MessageServiceInterface) {
                throw new \Exception(sprintf('The message service class %s must inherit and implement the "MessageServiceInterface" interface', $conf['class']));
            }
